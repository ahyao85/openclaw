...
describe("desktop panel audio wiring", () => {
  ...
  async function setup(audio = true, documentMode = false) {
  ...
  it.each(["source", "disconnect", "unmount", "tab-hidden"])(
    ...
    async (reason) => {
      ...
      clickPanelButton(panel, "[aria-label='Unmute desktop audio']");
      await panel.updateComplete;
      if (reason === "source") panel.requestedSource = "other";
      else if (reason === "disconnect") clickPanelButton(panel, "[aria-label='Disconnect']");
      else if (reason === "unmount") panel.remove();
      else {
  ...
  it("renders an actionable error without disconnecting the screen", async () => {
    ...
    AudioSocketMock.instances[0]!.onerror?.();
    await panel.updateComplete;
