...
describe("resolveSettledTurnFinalizationRequest", () => {
  ...
  it.each([
    { name: "active tool", patch: { itemLifecycle: { startedCount: 2, completedCount: 1, activeCount: 1 } } },
  ...
  ])("keeps $name under its existing owner instead of explaining a terminal warning", ({ patch }) => {
    const assistant = buildEmbeddedRunnerAssistant({ content: [{ type: "text", text: SILENT_REPLY_TOKEN }] });
    const attempt = makeEmbeddedRunnerAttempt({
      assistantTexts: [SILENT_REPLY_TOKEN],
      lastAssistant: assistant,
      currentAttemptAssistant: assistant,
    ...
    expect(resolveSettledTurnFinalizationRequest({
      runParams: { sessionId: "session:failure", runId: "run:failure", trigger: "heartbeat" } as never,
      attempt,
      activeErrorContext: { provider: "openai", model: "gpt-5.6-luna" },
      modelApi: "openai-responses",
      executionContract: undefined,
      payloadsWithToolMedia: [setReplyPayloadMetadata({ text: "A required command failed.", isError: true }, { toolErrorWarning: { toolName: "exec" } })],
      hasTerminalToolPresentation: false,
