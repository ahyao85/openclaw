...
import { getTopicCreatorUserId, resolveTopicNameCacheScope } from "./topic-name-cache.js";
...
async function createTopic(cfg: OpenClawConfig, creatorToken?: string) {
  ...
  await expect(
    getTopicCreatorUserId(
      groupChat.id,
      99,
      resolveTopicNameCacheScope(resolveStorePath(cfg.session?.store, { agentId: "main" })),
    ),
