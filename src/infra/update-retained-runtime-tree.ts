 ...
 * the filesystem refuses the link, a plugin boundary checks it, or relocation rewrites it.
...
export async function linkUpdateCandidatePluginTrees(
  ...
  for (const entry of plan.entries) {
    ...
    // Copy only plugin boundary inputs; ordinary payloads remain hardlinked.
    if (pluginFiles.has(entry.path) || isRelocatedFile(destination)) {
