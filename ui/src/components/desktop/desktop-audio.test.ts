...
describe("desktop audio connection", () => {
  ...
  function connect() {
    audio.connect(desktopAudioStream, "wss://gateway.test/base/");
  ...
  it.each(["mute", "close", "replace"])(
    ...
    async (action) => {
      ...
      else if (action === "close") audio.close();
      else connect();
      deferred.resolve();
      await Promise.resolve();
      ...
      if (action !== "mute") {
        lateMessage({ data: new ArrayBuffer(4) });
        lateError();
        lateOpen();
  ...
  it("reports unsupported and absent audio without opening a socket", () => {
    audio.connect(undefined, "wss://gateway.test");
    expect(audio.state).toBe("unavailable");
    vi.stubGlobal("AudioContext", undefined);
    audio.connect(desktopAudioStream, "wss://gateway.test");
