...
it.skipIf(process.platform !== "win32")(
  ...
  async () => {
    ...
    try {
      ...
      const observation = readWindowsServiceProcessObservation(process.env);
      expect(observation).not.toBeNull();
      if (!observation || child.pid === undefined) {
        throw new Error("Expected the live Unicode fixture and its native process snapshot");
      }
      const family = observation.findInstalled(18789, programArguments, false, child.pid);
      expect(family).toEqual([
        {
          pid: child.pid,
          parentPid: process.pid,
          startedAt: expect.stringMatching(/^[1-9]\d{0,18}$/u),
          programArguments,
        },
      ]);
      expect(() => observation.verify(child.pid)).not.toThrow();
    } finally {
