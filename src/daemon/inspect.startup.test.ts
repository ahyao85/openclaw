...
async function startup(
  ...
  const scriptPath = path.join(root, path.basename(startupPath), scriptName);
...
describe("Windows Startup service inventory", () => {
  it.each([
    { command: "gateway --port=19789", gateway: true, malformed: false },
    { command: '"gateway" --port=19789', gateway: true, malformed: false },
    { command: "gateway --port=19789 && echo extra", gateway: true, malformed: true },
    { command: "sync --gateway-url=https://example.invalid", gateway: false, malformed: false },
  ])(
    "inspects an unmarked custom Startup command: $command",
    async ({ command, gateway, malformed }) => {
      const { startupPath, scriptPath } = await startup("9.4", "\\Ops\\Runtime", "worker.cmd");
      await fs.writeFile(scriptPath, `@echo off\r\n"C:/Apps/openclaw.exe" ${command}\r\n`);
      expect(await findGatewayServices(environment())).toEqual({
        services:
          gateway && !malformed
            ? [
                expect.objectContaining({
                  label: path.basename(startupPath, ".vbs"),
                  marker: "openclaw",
                  windowsStartupEntry: startupPath,
                }),
              ]
            : [],
        errors: malformed ? [{ source: startupPath, message: expect.any(String) }] : [],
      });
    },
  );

  it.each(
    (["cmd", "9.2/9.3", "9.4"] as const).flatMap((form) =>
      ["cmd", "BaT"].map((extension) => ({ form, extension })),
    ),
  )(
    "discovers an unselected .$extension sibling from real $form launcher files",
    async ({ form, extension }) => {
      const { startupPath, taskName } = await startup(form, undefined, `gateway.${extension}`);
      expect(await findGatewayServices(environment())).toEqual({
        services: [
          expect.objectContaining({
            platform: "win32",
  ...
  it("discovers a generated Startup batch wrapper with a .BaT extension", async () => {
    const { startupPath, taskName } = await startup("cmd");
    const batchPath = startupPath.replace(/\.cmd$/i, ".BaT");
    await fs.rename(startupPath, batchPath);
    expect(await findGatewayServices(environment())).toEqual({
      services: [expect.objectContaining({ label: taskName, windowsStartupEntry: batchPath })],
      errors: [],
    });
  });
  ...
  it.each(
    ...
    "keeps Node Task and $form .$extension Startup identities distinct only in opt-in inventory",
    async ({ form, extension }) => {
      const { startupPath, scriptPath, taskName } = await startup(
        form,
        "OpenClaw Node",
        `node.${extension}`,
      );
      await fs.writeFile(
        ...
        [
          ...
          `set "OPENCLAW_WINDOWS_TASK_NAME=${taskName}"`,
          '"C:/Node/node.exe" "C:/Applications/openclaw/openclaw.mjs" node run --host example.invalid --port 18789',
          "",
        ].join("\r\n"),
      );
      vi.mocked(taskProbe.listScheduledTasks).mockReturnValue([
        {
          ...
          actions: [
            {
              type: 0,
              path: "C:/Applications/openclaw.exe",
              arguments: "node run --host example.invalid",
              workingDirectory: "",
            },
          ],
        },
      ]);
      expect(await findGatewayServices(environment())).toEqual({ services: [], errors: [] });
      const inventory = await findGatewayServices(environment(), { includeNode: true });
      expect(inventory.errors).toEqual([]);
      expect(inventory.services).toEqual([
        expect.objectContaining({
          label: taskName,
          scope: "system",
          detail: expect.stringContaining(`task: ${taskName}`),
        }),
        expect.objectContaining({
          label: taskName,
          scope: "user",
      ...
      expect(inventory.services[0]?.windowsStartupEntry).toBeUndefined();
    },
  );

  it.each(["cmd", "BaT"])(
    "retains recognizable failed Node .%s Startup launchers only in opt-in inventory",
    async (extension) => {
      const env = environment();
      const directory = path.dirname(resolveStartupEntryPath(env));
      await fs.mkdir(directory, { recursive: true });
      const unreadable = path.join(directory, `OpenClaw Node.${extension}`);
      const ambiguous = path.join(directory, "Opaque Node.vbs");
      await fs.mkdir(unreadable);
      await fs.writeFile(
        ambiguous,
        'CreateObject("WScript.Shell").Run "node C:\\Applications\\openclaw\\openclaw.mjs node run --host example.invalid", 0, False',
      );
      expect(await findGatewayServices(env)).toEqual({ services: [], errors: [] });
      const inventory = await findGatewayServices(env, { includeNode: true });
      expect(inventory.services).toEqual([]);
      expect(inventory.errors.map(({ source }) => source).toSorted()).toEqual(
        [unreadable, ambiguous].toSorted(),
      );
    },
  );
  ...
  it.each([false, true])(
    ...
    async (taskExists) => {
      ...
      vi.stubEnv("OPENCLAW_TASK_SCRIPT", selected.scriptPath);
  ...
  it.each([false, true])(
    "records a Task that disappears after enumeration (fallback=%s)",
    async (fallback) => {
      const { startupPath, scriptPath, taskName } = await startup("9.4");
      if (!fallback) {
        await fs.rm(startupPath);
      }
      vi.mocked(taskProbe.listScheduledTasks)
        .mockReturnValueOnce([
          {
            taskPath: taskName,
            state: 3,
      ...
      const services = fallback
        ...
        : [];
      await expect(findGatewayServices(environment())).rejects.toBeInstanceOf(
        taskProbe.WindowsServiceObservationChangedError,
      );
      expect(await findGatewayServices(environment())).toEqual({ services, errors: [] });
    },
  );
  ...
  it.each(["entry", "script"] as const)(
    ...
    async (changed) => {
      ...
      ).rejects.toBeInstanceOf(taskProbe.WindowsServiceObservationChangedError);
    },
  );

  it("reports temporary Startup re-read failure without declaring an ownership change", async () => {
    const { startupPath, scriptPath } = await startup("9.4");
    const readFile = fs.readFile;
    let scriptReads = 0;
    vi.spyOn(fs, "readFile").mockImplementation((...args) => {
      if (args[0] === scriptPath && ++scriptReads === 2) {
        return Promise.reject(
          Object.assign(new Error("Synthetic access failure"), { code: "EACCES" }),
        );
      }
    ...
    expect(await findGatewayServices(environment())).toEqual({
      services: [],
      errors: [{ source: startupPath, message: "Startup launcher could not be inspected." }],
    });
  });
