...
describe("restart sentinel notice recovery", () => {
  ...
  it("serializes stable notice preparation before modifiers can run twice", async () => {
    ...
    try {
      // Observe hook entry without imposing a separate deadline on SQLite preparation.
      // Race the producer so a preparation failure cannot strand the entry wait.
      await Promise.race([modifierEntered.promise, first]);
      expect(mocks.hookRunner.runMessageSending).toHaveBeenCalledOnce();
      let secondSettled = false;
      second = enqueueRestartSentinelNotice(request).finally(() => {
        secondSettled = true;
      });
      ...
      // An assertion failure must not let database teardown overtake either enqueue.
      releaseModifier.resolve();
