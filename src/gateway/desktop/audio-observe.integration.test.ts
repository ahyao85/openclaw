...
describe("audio tied to the actual RFB observer", () => {
  it.each(["success", "rejected", "released", "expired", "stale-owner"] as const)(
    ...
    async (outcome) => {
      if (outcome === "expired") vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout"] });
      ...
      serverStream.on("data", () => {
        if (upstreamStage++ === 0) serverStream.write(Buffer.from([1, 2]));
        else if (upstreamStage === 2) serverStream.write(Buffer.alloc(16, 1));
        else authenticated.resolve();
      });
      const screen = new WebSocket(origin + "/desktop/observe?token=" + grant.token);
      cleanups.push(() => screen.terminate());
      let browserStage = 0;
      screen.on("message", () => {
        if (browserStage++ === 0) screen.send(version);
        else if (browserStage === 2) screen.send(Buffer.from([1]));
      });
