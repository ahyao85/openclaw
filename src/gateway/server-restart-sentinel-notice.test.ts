...
describe("restart sentinel notice recovery", () => {
  ...
  it("serializes stable notice preparation before modifiers can run twice", async () => {
    ...
    try {
      // Worker admission precedes the modifier; elapsed time is not an entry signal.
      await Promise.race([
        modifierEntered.promise,
        first.then(() => {
          throw new Error("notice preparation completed without entering its modifier");
        }),
      ]);
      ...
      releaseModifier.resolve();
      // An assertion failure must not let teardown close either producer's database.
