...
Open **Home** from the sidebar's **Pages** menu to open the active agent's main session, just like the Control UI, or select another session from the sidebar. Home uses the Gateway's configured main key, including global session scope; it does not change Android's device-session or Talk binding.

The default page order is **Home**, **Sessions**, **Overview**, **Skills**, **Settings**. Saved custom orders stay unchanged; **Reset pinned items** restores this default order.

**Overview** shows the selected agent, one Gateway connection chip, Home and Talk actions, a compact Runs/Automations/Devices/Approvals strip, and recent sessions excluding the agent’s main session. Tap the agent to manage agents, the Gateway chip for connection settings, or any status column for its corresponding list. **Talk** requests microphone permission and checks setup before opening Home and starting Talk in that session; it does not replace the device-owned default used by other voice entry points.

**Needs you** appears only for actionable warnings, including approvals, failed or overdue enabled automations, channel errors, device access requests, unavailable providers, and a disconnected Gateway. Warnings remain available even when many need attention. Overview reduces its recent rows when space is tight and stays scrollable for large text and small windows. **See all** opens Sessions; its count includes the main session and uses the Gateway’s total when available. A partial loaded list without a trustworthy total shows a lower bound such as **200+**, rather than an exact capped count. **Files** opens the agent workspace.

On **Sessions**, the search field and **Session options** button sit above the Recent, Current, and Archived filters. Session options contains checked **Sort** choices (Newest first / Oldest first) and **Layout** choices (Compact / Detailed); these change the same list without changing its filter or search query.
...
- Session selection: while the app is running, each Gateway and agent remembers the last chat you explicitly selected, including an explicit **Home** selection. Returning to an agent checks an older chat directly if it is outside the recent page; temporary lookup failures show an error without forgetting that choice.
