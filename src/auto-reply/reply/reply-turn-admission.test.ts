...
describe("reply turn admission", () => {
  ...
  it.each(["initial", "later"] as const)(
    "accepts an expected session id rotated by the active run (%s predecessor)",
    async (predecessor) => {
      ...
      const storePath = createSessionStoreFor(sessionKey, sessionId);
      let active: ReplyOperation | undefined;
      let registeringPredecessor = false;
      const registerPredecessor = async () => {
        registeringPredecessor = true;
        try {
          active = await admitTestReplyOperation({ sessionKey, sessionId, storePath });
          active.setPhase("preflight_compacting");
        } finally {
          registeringPredecessor = false;
        }
      };
      if (predecessor === "initial") {
        await registerPredecessor();
      }
      ...
      const loadSpy = vi
        ...
        .mockImplementation(async (...args) => {
          if (registeringPredecessor) {
            return await load(...args);
          }
          const read = ++reads;
          const snapshot = await load(...args);
          if (read === 1 && predecessor === "later") {
            expect(replyRunRegistry.get(sessionKey)).toBeUndefined();
            await registerPredecessor();
          }
          // Hold a real result captured while the observed predecessor still owns the lane.
          if (read === 2) {
      ...
      const admitted = admitTestReplyTurn({
        ...
        expectedSessionId: sessionId,
        storePath,
        kind: predecessor === "later" ? "queued_followup" : "visible",
      });
