...
import { recordTopicCreation, resolveTopicNameCacheScope } from "./topic-name-cache.js";
  ...
  return withTelegramApiContext(
    ...
    async (context): Promise<TelegramCreateForumTopicResult> => {
      ...
      await recordTopicCreation(
        ...
        resolveTopicNameCacheScope(resolveStorePath(cfg.session?.store, { agentId: ownerAgentId })),
      ).catch((err: unknown) => {
